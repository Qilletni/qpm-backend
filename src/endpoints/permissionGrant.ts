 ...
 * POST /permissions/:user/:permission
...
import { isAdmin, grantPermission } from "../utils/permissions";

// Success response schema
const PermissionGrantResponse = z.object({
	success: z.literal(true),
	message: Str({
		example: "Permission 'admin' granted to user 12345",
		description: "Success message"
	}),
	user: z.number({
		description: "GitHub user ID that received the permission"
	}),
	permission: Str({
		example: "admin",
		description: "Permission that was granted"
	}),
});

export class PermissionGrant extends OpenAPIRoute {
	schema = {
		tags: ["Permissions"],
		summary: "Grant permission to user",
		description:
			"Grant a permission (e.g., 'admin') to a user by their GitHub user ID. Only admins can use this endpoint.",
		request: {
			params: z.object({
				user: Str({
				...
				permission: Str({
					description: "Permission name to grant (e.g., 'admin')",
					example: "admin",
				}),
			}),
			headers: z.object({
		...
		responses: {
			"200": {
				...
				...contentJson(PermissionGrantResponse),
			},
			"401": {
	...
	async handle(c: AppContext) {
		...
		await grantPermission(userId, permission, c.env);
